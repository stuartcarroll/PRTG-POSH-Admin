function Resume-PrtgAllSensorsOn
{
	Param(
    ...
    elseif ($Device -and !$DeviceGroup -and $Probe)
    {
        ...
        foreach ($Sensor in $Sensors) 
        {
            ...
            if ($Simulate -eq "on"){
                write-host "Resuming Sensor Name:"
                write-host $SenName
                write-host "Resuming Sensor ID:"
                write-host $SenObjid
                }
                else
                {
                ...
                $url = "https://$PRTGHost/api/pause.htm?id=$SenObjid&action=1&$auth"
